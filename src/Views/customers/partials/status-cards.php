...
<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5 mb-6">
    <!-- Total -->
    <a href="/customers" class="card card-status p-4 <?= empty($currentStatus) ? 'ring-2 ring-blue-500' : '' ?>">
    ...
    <!-- New -->
    <a href="/customers?status=<?= Customer::STATUS_NEW ?>" class="card card-status p-4 <?= ($currentStatus ?? '') === Customer::STATUS_NEW ? 'ring-2 ring-green-500' : '' ?>">
        <div class="flex items-center">
            <div class="flex-shrink-0">
    ...
    <!-- Contacted -->
    <a href="/customers?status=<?= Customer::STATUS_CONTACTED ?>" class="card card-status p-4 <?= ($currentStatus ?? '') === Customer::STATUS_CONTACTED ? 'ring-2 ring-blue-500' : '' ?>">
        <div class="flex items-center">
            <div class="flex-shrink-0">
                <div class="h-10 w-10 rounded-lg bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center">
                    <svg class="h-5 w-5 text-blue-600 dark:text-blue-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    <!-- Callback -->
    <a href="/customers?status=<?= Customer::STATUS_CALLBACK ?>" class="card card-status p-4 <?= ($currentStatus ?? '') === Customer::STATUS_CALLBACK ? 'ring-2 ring-yellow-500' : '' ?>">
        <div class="flex items-center">
            <div class="flex-shrink-0">
    ...
    <!-- Follow Up -->
    <a href="/customers?status=<?= Customer::STATUS_FOLLOW_UP ?>" class="card card-status p-4 <?= ($currentStatus ?? '') === Customer::STATUS_FOLLOW_UP ? 'ring-2 ring-purple-500' : '' ?>">
        <div class="flex items-center">
            <div class="flex-shrink-0">
